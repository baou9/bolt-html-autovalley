...
/* ============================================================
   Services Excellence – Perspective Grid Background
   ============================================================ */
(() => {
  const ICONS = ["#svc-ico-wrench","#svc-ico-gear","#svc-ico-car","#svc-ico-battery","#svc-ico-brake"];
  const rmq = window.matchMedia('(prefers-reduced-motion: reduce)');
  const mmq = window.matchMedia('(max-width: 680px)');
  const rand = (arr) => arr[(Math.random() * arr.length) | 0];

  const section = document.getElementById('services-excellence');
  if (!section) return;

  const grid = section.querySelector('[data-av-grid]');
  const svg  = section.querySelector('[data-av-svg]');
  if (!grid || !svg) return;
  ...
  function getGS() {
    return mmq.matches ? 68 : 60;
  }

  function getCount() {
    const rect = section.getBoundingClientRect();
    const base = mmq.matches ? 22 : 44;
    const areaFactor = Math.min(1.25, Math.max(0.85, (rect.width * rect.height) / (1100 * 650)));
  ...
  function build() {
    ...
    const rect = section.getBoundingClientRect();
    W = Math.max(1, Math.floor(rect.width * 3));
    H = Math.max(1, Math.floor(rect.height * 3));

    svg.setAttribute('viewBox', `0 0 ${W} ${H}`);
    svg.setAttribute('preserveAspectRatio', 'none');

    const gs = getGS();
    cols = Math.max(1, Math.floor(W / gs));
    rows = Math.max(1, Math.floor(H / gs));
    ...
    for (let i = 0; i < count; i++) {
      ...
      g.style.animationDuration = (4.2 + Math.random() * 4).toFixed(2) + 's';
      g.style.animationDelay = (Math.random() * -10).toFixed(2) + 's';

      const r = document.createElementNS("http://www.w3.org/2000/svg", "rect");
      r.setAttribute('width', gs - 2);
      r.setAttribute('height', gs - 2);
      g.appendChild(r);

      const iconSize = 26;
      const offset = (gs - iconSize) / 2;

      const iconSvg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
      iconSvg.setAttribute('class', 'svc-icon');
      iconSvg.setAttribute('x', offset);
      iconSvg.setAttribute('y', offset);
      ...
      use.setAttribute('href', rand(ICONS));
      use.setAttribute('xlink:href', use.getAttribute('href'));
      ...
      const obj = { wrapper, use, x: 0, y: 0, gs };
      teleport(obj);
      g.addEventListener('animationiteration', () => teleport(obj));
      cells.push(obj);
    }
  ...
  function teleport(obj) {
    const gs = obj.gs;
    obj.x = ((Math.random() * cols) | 0) * gs;
    const baseRow = ((Math.random() * rows) | 0);
    obj.y = (baseRow * gs) + shift;
    obj.use.setAttribute('href', rand(ICONS));
    obj.use.setAttribute('xlink:href', obj.use.getAttribute('href'));
    obj.wrapper.setAttribute('transform', `translate(${obj.x},${obj.y})`);
  }
  ...
  function tick(t) {
    ...
    if (!rmq.matches) {
      const gs = cells[0]?.gs || 60;
      shift = (shift + speed * dt) % gs;
      grid.style.transform = `perspective(1000px) rotateX(65deg) translateY(${shift}px)`;
      for (const c of cells) {
        c.y += speed * dt;
        c.wrapper.setAttribute('transform', `translate(${c.x},${c.y})`);
      }
    }
  ...
  function start() {
    if (raf) return;
    section.classList.remove('is-bg-paused');
  ...
  function stop() {
    section.classList.add('is-bg-paused');
  ...
  window.addEventListener('resize', () => {
    clearTimeout(rt);
    rt = setTimeout(() => {
      if (section.classList.contains('is-bg-paused')) { needsRebuild = true; return; }
      stop(); build(); start();
    }, 220);
  }, { passive: true });

  mmq.addEventListener?.('change', () => {
    if (section.classList.contains('is-bg-paused')) { needsRebuild = true; return; }
    stop(); build(); start();
  });
